...
describe('flow',{parallel:true}, function () {
  ...
  .add('get:array', function (msg, done) {
    ...
    done(null, [{item:1},{item:2},{item:3},{item:4}]);

  })

  .add('get:array2', function (msg, done) {

    done(null, [{item:4},{item:5},{item:6},{item:7}]);

  })

  .add('get:array_complex', function (msg, done) {

    done(null, [{item:1, ok:false, nested:{ active:true}},{item:2, ok:true, nested:{ active:true}},{item:3, ok:false,nested:{ active:false}},{item:4, ok:false,nested:{ active:true}}]);
  ...
  it('iterate, exit', function (done) {
    ...
    si.flow()
      ...
      .end(function (err, out) {
        ...
        if (err) return done(err);
        expect(out).to.equal([{ returnindex: 0 }, { returnindex: 1 }, { returnindex: 2 }])
        done();
      })
  });
    ...
    var act = {
      sequence: [{
        ...
        sequence:[
          ...
          { get:'array',
            out$:[
              { each:{ _:'padEnd', args:[3, '-'], select:'item' } },
              {  _:'find', args:{ item:'1--' } }
            ]
          },
          { get:'xyz' },
  ...
  it('merge arrays in sequence', function (done) {
    ...
    var act = {
      merge:true,
      sequence:[
        { get:'array_complex', key$:'to_merge'},
        { get:'array2', key$:'to_merge'}
      ]
    };
    si.flow()
      .wait(act)
  ...
  it('iterate must have times or with', function (done) {
    ...
    var act = {
      iterate: { cmd:'echo', hello:true }
    };
    si.flow(function (err, out) {
        expect(err.message).to.be.equal(`seneca: Action iterate:* failed: Exactly one of these properties must be used: \'times,with\' (parent: top level)..`);
        done();
      })
      .wait(act)
      .end()
